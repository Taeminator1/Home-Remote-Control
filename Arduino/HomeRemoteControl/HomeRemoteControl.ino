...
bool startFlag = false;                   // Flag to store the internal states of toggles when NodeMCU is rebooted
        ...
        if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {
          ...
          for (int i = 0; i < payload.length() - target.length(); i++) {
            if (payload.substring(i, i + target.length()) == target) {
              ...
              if (buff == "true ")  { toggles[toggleIndex++].setEState(true); }
              else                  { toggles[toggleIndex++].setEState(false); }
          ...
          if (startFlag) {
            ...
            for (int i = 0; i < BUTTONS_COUNT; i++)
              toggles[i].syncIStateWithEState();
            startFlag = true;
          }
        }
      } else {
